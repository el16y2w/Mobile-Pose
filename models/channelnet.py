...
def channelwise_conv2d(x,
    ...
    x = tf.keras.layers.Conv3D(
        ...
        data_format=data_format,
        use_bias=False,
        name=name + '/conv')(x) #(?,7,14,512,2)
    if dropout_rate > 0.0:
        x = tf.keras.layers.Dropout(
            rate=dropout_rate,
            name=name + "/dropout")(
            inputs=x,
            training=training) #(?,7,14,512,2)
    if filters == 1:
...
def conv_group_block(x,
    ...
    Tensor
        Resulted tensor.
    """
    assert (channels == x.shape[3].value)
...
def channet_unit(x,
    ...
    for i, (out_channels, block_name) in enumerate(zip(out_channels_list, block_names)):
        ...
        assert (x.shape[3].value == in_channels)
...
class ChannelNet(object):
    ...
    def __init__(self,
        ...
        self.training = config.isTrain


    def __call__(self,
                 x,
                 training=config.isTrain):
        """
        Build a model graph.
...
class ChannelNetget:
    ...
    """
    def __init__(self,shape, model_name=None,pretrained=False,groups=2, width_scale=1.0, root=os.path.join("~", ".tensorflow", "models")):
        self.groups = groups

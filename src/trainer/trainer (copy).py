...
class Trainer:
    ...
    def start(self, fromStep, totalSteps, lr, modeltype, time):
        ...
        result = open(os.path.join(exp_dir, opt.backbone + time + "training_result.csv"), "w")
        result.write(
        ...
        for i in range(fromStep, fromStep + totalSteps + 1):
            ...
            result = open(os.path.join(exp_dir, opt.backbone + time + "training_result.csv"), "a+")
            ...
            # 每100轮迭代输出状态
            if (total_iterations % 10 == 0) or (i == totalSteps - 1):

...
class AnnotatorInterface:
    ...
    def __init__(self, bbox_model, pose_2d_model, max_persons):
    ...
    """
    @staticmethod
    def build(model,inputsize,max_persons=1):

        bbox_model = ModelFactory.build_object_detection_interface()
        pose_2d_model = ModelFactory.build_pose_2d_interface(model)

        return AnnotatorInterface(bbox_model, pose_2d_model,  max_persons)
    ...
    def jsonify(self):
        ...
        for i in range(len(persons)):
            ...
            annot = {'id':persons[i]['id'], 'pose_2d': {}, 'confidence':persons[i]['confidence'].tolist()}
